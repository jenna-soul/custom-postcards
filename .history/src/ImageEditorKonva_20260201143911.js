...
/*Handle printing */
// Generates a high-res canvas for printing
const getPrintCanvas = () => {
  if (!imageObj) return null;

  const width = orientation === "portrait" ? 800 : 1200;  // 4x6 at 300dpi ratio
  ...
  // Draw uploaded image scaled to cover entire canvas
  const scale = Math.max(width / imageObj.width, height / imageObj.height);
  const x = (width - imageObj.width * scale) / 2;
  const y = (height - imageObj.height * scale) / 2;
  ctx.drawImage(imageObj, x, y, imageObj.width * scale, imageObj.height * scale);

  // Draw frame if it exists
  if (frameObj) {
    ctx.drawImage(frameObj, 0, 0, width, height);
  }
...
const handlePrint = () => {
  ...
  printableDiv.innerHTML = ""; // clear Konva Stage for printing
  printableDiv.appendChild(printImg);
