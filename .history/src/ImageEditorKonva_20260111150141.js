...
{frameObj && (() => {
  // Calculate scale and position to preserve aspect ratio (cover the canvas)
  const scaleX = CANVAS_WIDTH / frameObj.width;
  const scaleY = CANVAS_HEIGHT / frameObj.height;
  const scale = Math.max(scaleX, scaleY); // "cover" behavior

  const x = (CANVAS_WIDTH - frameObj.width * scale) / 2;
  const y = (CANVAS_HEIGHT - frameObj.height * scale) / 2;
  ...
  return (
    <KonvaImage
      ...
      x={x}
      y={y}
      width={frameObj.width * scale}
      height={frameObj.height * scale}
  ...
  <Frames onSelectFrame={(imgSrc) => loadFrame(imgSrc)} />
