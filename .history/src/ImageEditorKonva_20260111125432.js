...
const fitImageToFrame = (img) => {
  ...
  const minX = CANVAS_WIDTH - imgWidth;
  const minY = CANVAS_HEIGHT - imgHeight;
...
img.onload = () => {
  ...
  setImageScale((prev) => {
    ...
    const pos = clampPosition(imagePosition.x, imagePosition.y, next);
  ...
  const zoomOut = () => {
        ...
        const scaleX = CANVAS_WIDTH / imageObj.width;
        const scaleY = CANVAS_HEIGHT / imageObj.height;
        const MIN_COVER_SCALE = Math.max(scaleX, scaleY);

        setImageScale((prev) => {
          const next = Math.max(prev / 1.2, MIN_COVER_SCALE);
          const pos = clampPosition(imagePosition.x, imagePosition.y, next);
          setImagePosition(pos);
          return next;
    });
          ...
          <Layer>
            {imageObj && ( //Uploaded image
              <KonvaImage
                ...
                draggable
                onDragMove={(e) => {
                  const { x, y } = clampPosition(e.target.x(), e.target.y());
                  setImagePosition({ x, y });
                }}
              />
            )}
